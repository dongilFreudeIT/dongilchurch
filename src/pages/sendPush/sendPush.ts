...
export class SendPushPage {
  ...
  constructor(public navCtrl: NavController, public alertCtrl: AlertController, public navParams: NavParams, public http: HTTP, private storage: Storage,
    ...
    private loader: LoaderProvider, public toastController: ToastController, private network: Network, private chechNetwork: CheckNetworkProvider
  ) {
        ...
        this.http.post(this.url + '/push/send_push', param, {}).then(data => {
          if (data.status == 200) {
            ...
            if (obj.code == "S01") {
              ...
              let alert = this.alertCtrl.create({
                title: "성공",
                subTitle: "총 " + successCount + "명에서 메시지를 전송했습니다.",
                buttons: ['OK']
              });
              this.loader.hide();
              alert.present();
