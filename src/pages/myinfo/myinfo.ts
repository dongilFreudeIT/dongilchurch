...
export class MyinfoPage {
  ...
  update(){
    ...
    //   this.showAlert("알림","입력하신 2개의 비밀번호가 일치하지 않습니다.");
    //   return;
    // }
    ...
    if(this.user.phone == null || this.user.phone == ''|| this.user.phone.length !=11){
    ...
    if(this.user.birthday != null){
      ...
      var today = new Date();
      var birth = new Date(this.user.birthday);
      if(birth  >=  today){
        this.showAlert("알림", "생년월일 입력이 잘못 되었습니다.");
        return;
      }
      var tempBirthday = this.user.birthday;
      this.user.birthday = tempBirthday.replace(/-/gi, "");
    }
    var param = {
