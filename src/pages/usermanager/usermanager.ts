      ...
      this.userArray = this.userArray.filter(item => {
        ...
        if (
          (data.place != null && data.place != ""
            ? tempPlace != null && tempPlace.includes(data.place)
            : true) &&
          (data.subgroup != null && data.subgroup != ""
            ? tempGroup != null && tempGroup.includes(data.subgroup)
            : true) &&
          (data.title != null && data.title != ""
            ? tempTitle != null && tempTitle.includes(data.title)
            : true) &&
          (data.grade != null && data.grade != ""
            ? tempGrade != null && tempGrade.includes(data.grade)
            : true) &&
          (data.subtitle != null && data.subtitle != ""
            ? tempSubtitle != null && tempSubtitle.includes(data.subtitle)
            : true)
        ) {
          return true;
        } else {
      ...
      for (i = 0; i < this.userArray.length; i++) {
        ...
        if (
          (data.place === undefined ||
            data.place === "" ||
            data.place === this.userArray[i].place) &&
          (data.subgroup === undefined ||
            data.subgroup === "" ||
            data.subgroup === this.userArray[i].subgroup) &&
          (data.subtitle === undefined ||
            data.subtitle === "" ||
            data.subtitle === this.userArray[i].subtitle) &&
          (data.title === undefined ||
            data.title === "" ||
            data.title === this.userArray[i].title) &&
          (data.grade === undefined ||
            data.grade === "" ||
            data.grade === this.userArray[i].grade)
        ) {
  ...
  sendPush() {
    ...
    this.storage.get("user_serial").then(localSerial => {
      const browser = this.iab.create(
        this.url+"/SendPush?serial=" + localSerial,
        "target='_blank'"
      );
      browser.show();
    });
  ...
  searchUser(event: any) {
    ...
    if (val && val.trim() != "") {
      this.userArray = this.userArray.filter(item => {
        ...
        var tempId = item.id;
        if (
          (tempName != null && tempName.includes(val)) ||
          (tempPhone != null && tempPhone.includes(val)) ||
          (tempId != null && tempId.includes(val))
        ) {
          return true;
        } else {
          return false;
